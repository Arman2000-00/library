...
public class BookServiceImpl implements BookService {
    ...
    public void save(BookRequest bookRequest, CurrentUser currentUser) {
        UserResponse byId = userService.findById(currentUser.getUser().getId());
        byId.getBooks().add(modelMapper.map(bookRequest, Book.class));
        bookRepository.save(bookMapper.toEntity(bookRequest));
        userService.save(modelMapper.map(byId, UserRequest.class));
    }
